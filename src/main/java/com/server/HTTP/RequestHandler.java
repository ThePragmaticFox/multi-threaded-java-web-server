...
public class RequestHandler {
    ...
    private static Optional<Header> parseHeader(final WebServerConfig webServerConfig, final List<String> headerLines,
        ...
        if (headerLines.size() < 1) {
            final String statusCode = StatusCode.BAD_REQUEST.getString(Version.HTTP_1_1);
        ...
        if (version.isEmpty()) {
            final String statusCode = (StatusCode.HTTP_VERSION_NOT_SUPPORTED.getString(Version.HTTP_1_1));
            ResponseWriter.headerError(statusCode, outputStream);
            return Optional.empty();
        }
